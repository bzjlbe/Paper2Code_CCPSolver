...
subplot('position',[0.05,0.1,0.42,0.84]);
    ...
    plot(time1,q1(1,:),'-.r',LineWidth=2)
    plot(time2,q2(1,:),'--g',LineWidth=2)
    
    xlabel('t/s');ylabel('Tangential displacement(m)');ytickformat('%.2f')
    legend('Reference','CCP\_Wang','Proposed method','location','best');
       
    rectangle('Position',[0.4, 0.37, 0.15, 0.02], 'EdgeColor','black','Linewidth',2,'LineStyle','--');
        axes('position', [0.25 0.3 0.2 0.3]);
        box on;hold on
        plot(t_Ana,diap,'k',LineWidth=2)
        plot(time1,q1(1,:),'-.r',LineWidth=2)
        plot(time2,q2(1,:),'--g',LineWidth=2)
...
subplot('position',[0.55,0.1,0.42,0.84]);
    ...
    plot(time1,dq1(1,:),'-.r',LineWidth=2)
    plot(time2,dq2(1,:),'--g',LineWidth=2)
